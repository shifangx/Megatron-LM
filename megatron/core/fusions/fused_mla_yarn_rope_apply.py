...
def _mla_rope_fwd_kv_split_kernel(
    ...
    # The head-block offset lives in the index, not the base pointer, so the mask is
    # able to drop the out-of-range heads of the last block. k_dim is padded up to a
    # power of two because tl.arange requires it, and the padding is masked off.
...
def _mla_rope_bwd_kv_split_kernel(
    ...
    if pid_head == 0:
        ...
        for i in tl.static_range(triton.cdiv(head_num, BLOCK_H)):
            dK_ptr = dK + pid_m * stride_dk_seq
            x_i_range = tl.arange(0, BLOCK_H)[:, None] + i * BLOCK_H
            x_off = x_i_range * stride_dk_nheads + k_dim
            mask = x_i_range < head_num
            if REMOVE_INTERLEAVING:
                x_1_off = x_off + tl.arange(0, emb_dim // 2)[None, :] * 2
                x_2_off = x_1_off + 1
