...
def get_module_to_grid_tuple(mimo_model, vision_module_grid, language_module_grid):
    return_tuple = [
        (mimo_model.modality_submodules['images'], vision_module_grid),
        (mimo_model.language_model, language_module_grid),
    ]
    return return_tuple
...
def _create_pg_collection(
    ...
    if embedding_rank_lists:
        embd_pg, _ = torch.distributed.new_subgroups_by_enumeration(
            embedding_rank_lists, backend="nccl"
        )
    if pos_embedding_rank_lists:
        pos_embd_pg, _ = torch.distributed.new_subgroups_by_enumeration(
            pos_embedding_rank_lists, backend="nccl"
        )
