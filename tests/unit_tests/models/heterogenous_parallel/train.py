...
from tests.unit_tests.models.heterogenous_parallel.data import get_batch, get_data_iterator
...
def test_1f_1b_schedule_vlm_mimo_model_custom_pgs(
    ...
    mimo_model, module_to_grid_map, topology, vision_pg_collection, language_pg_collection = (
        get_vlm_mimo_model(
    ...
    module_to_grid_tuple = get_module_to_grid_tuple(
        mimo_model, module_to_grid_map['images'], module_to_grid_map['language_module']
    )

    # Configure no_sync and finalize_model_grads functions
    mimo_model.config.no_sync_func = partial(
        multimodule_no_sync, module_to_grid_tuple=module_to_grid_tuple
    )
    mimo_model.config.finalize_model_grads_func = partial(
        finalize_model_grads, module_to_grid_tuple=module_to_grid_tuple
    )
    ...
    for iteration in range(num_iterations):
        ...
        print(
            f"for debug: Rank {dist.get_rank()}, at iteration {iteration}, update_successful: {update_successful}, grad_norm: {grad_norm}, num_zeros_in_grad: {num_zeros_in_grad}"
        )
...
if __name__ == "__main__":
    ...
    language_tp, language_pp, language_dp, language_cp, language_ep = 1, 2, 1, 1, 1
    assert vision_cp == 1, f"Do not support vision module with CP > 1 currently"
    assert language_cp == 1, f"Do not support language module with CP > 1 currently"
    ...
    if rank < vision_tp * vision_pp * vision_dp:
        assert (
            global_batch_size % (num_microbatches * vision_dp) == 0
        ...
        assert (
            global_batch_size % (num_microbatches * language_dp) == 0
