...
def _collate_fn(
    batch: List[Dict], image_seq_length: int = 1024, hidden_size: int = 1024
            ...
            "images": {
                "clip_encoder": {
                    'hidden_states': torch.randn(
                        image_seq_length, bsz, hidden_size, dtype=torch.bfloat16
                    ),
                    'attention_mask': None,
                }
            }
...
def get_data_iterator(
    ...
    llm_condition = is_current_rank_in_grid(llm_grid) and (
    ...
    if encoder_1_condition or llm_condition:
        ...
        dataloader = DataLoader(
            ...
            collate_fn=lambda batch: _collate_fn(
                batch, image_seq_length=image_seq_length, hidden_size=vision_hidden_size
            ),
        )
